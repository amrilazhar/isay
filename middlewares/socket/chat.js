...
async function startSocketChat(req, res) {
	try {
		...
		req.socket.on("readMessage", async (data) => {
			...
			req.io.to(req.profile.id.toString()).emit("readedChat:" + req.profile.id, data.message_id);
		});
			...
			if (message.message_type !== "text") {
				...
				chat.message
					...
					.then(async (query) => {
						...
						req.io.to(message.to.toString()).emit("chat:" + message.to, sendMess);
	...
	try {
		...
		chat.message
			...
			.then(async (query) => {
				...
				req.io.to(req.utils.message.to.toString()).emit("chat:" + req.utils.message.to, sendMess);
